...
function Base.:<=(A::WeilDivisor, B::WeilDivisor)
...
@doc raw"""
    subsystem(L::LinearSystem, E::WeilDivisor, n::Int) -> LinearSystem, MatElem

Given a linear system ``L = |E|`` and a divisor ``D \leq E`` compute ``|D|``
and the inclusion ``|D| \hookrightarrow |E|``.

The inclusion is given in terms of a matrix on the given bases of the systems.
"""
function subsystem(L::LinearSystem, D::WeilDivisor)
  ...
  for P in components(E)
    #n = coeff(E, P) - coeff(D,P)
    #if n == 0
    #  continue
    #end
    @show P
    Lnew, Tnew = _subsystem(L, P, -coeff(D,P))
    @show gens(Lnew)
    T = Tnew*T
  end
  return Lnew, T
...
@doc raw"""
    _subsystem(L::LinearSystem, P::IdealSheaf, n::Int) -> LinearSystem

Given a linear system ``L = |D|``, a sheaf of prime ideals `P` 
and an integer `n`, return a pair ``(K, A)`` consisting
of the subsystem of elements in ``|D'|`` and the representing
