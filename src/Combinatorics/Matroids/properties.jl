...
function rank(M::Matroid, set::GroundsetType)
    if length(set)==0
        ...
        return Polymake.matroid.rank(pm_object(M), _gs_to_indices(set, M.gs2num; offset=-1, type=Set))::Int
    end
end
...
```
"""
nullity(M::Matroid, set::GroundsetType) = length(set)-Polymake.matroid.rank(pm_object(M), _gs_to_indices(set, M.gs2num; offset=-1, type=Set))::Int
...
_revlex_basis_from_vector(v::AbstractVector{Int}) = join(isone(x) ? '*' : '0' for x in v)
